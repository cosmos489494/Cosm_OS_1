...
#             video.asm              (driver de video - binario flat)
#             interface.asm          (interface - binario flat)
#   saidas  : inicio.obj            (na raiz do projeto)
#             EFI/Boot/bootx64.efi  (na raiz do projeto)
#             kernel-1.5.2026       (na raiz do projeto - SEPARADO)
#             video.dr              (na raiz do projeto - SEPARADO)
#             interface.grain       (na raiz do projeto - SEPARADO)
...
SRC="inicio.asm"
KRN_SRC="kernel.asm"
KRN="kernel-1.5.2026"          # o nome carrega a versao da build
VID_SRC="video.asm"
VID="video.dr"                 # driver de video - arquivo SEPARADO
ITF_SRC="interface.asm"
ITF="interface.grain"          # interface - arquivo SEPARADO
...
echo "== [1/6] NASM: $SRC -> inicio.obj (COFF x64)"
nasm -f win64 "$SRC" -o inicio.obj

echo "== [1/6] LINKER: inicio.obj -> $OUT_EFI (.efi UEFI)"
mkdir -p EFI/Boot
lld-link /machine:x64 /subsystem:efi_application /entry:inicio \
         /out:"$OUT_EFI" inicio.obj

echo "== [2/6] NASM: $KRN_SRC -> $KRN (binario flat - arquivo SEPARADO)"
nasm -f bin "$KRN_SRC" -o "$KRN"

echo "== [3/6] NASM: $VID_SRC -> $VID (driver de video - binario flat)"
nasm -f bin "$VID_SRC" -o "$VID"

echo "== [4/6] NASM: $ITF_SRC -> $ITF (interface - binario flat)"
nasm -f bin "$ITF_SRC" -o "$ITF"

echo "== [5/6] Montando imagem FAT da ESP (só o bootx64.efi, El Torito UEFI)"
...
echo "== [6/6] XORRISO: gerando $ISO (EFI/Boot + $KRN + $VID + $ITF lado a lado)"
...
echo "  VIDEO : $VID ($(stat -c%s "$VID") bytes)"
echo "  ITF   : $ITF ($(stat -c%s "$ITF") bytes)"

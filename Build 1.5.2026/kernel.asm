; Cosm OS 1 - kernel (Build 1.5.2026)
...
;
; Formato do arquivo "kernel-1.5.2026":
...
;
; O que o kernel faz nesta build (1.5.2026 - fluxo aprovado):
;   1) limpa a tela e escreve "kernel-1.5.2026";
...
; ---------------- primeira pintura: so a linha do kernel ----------------
    ...
    mov   rcx, rdi                ; OutputString("kernel-1.5.2026")
    lea   rdx, [rel texto1]
    call  qword [rdi + 8]
...
; -------------- as DUAS mensagens na tela por 5 SEGUNDOS --------------
    ...
    mov   rcx, rdi                ; OutputString("kernel-1.5.2026" + CRLF)
    lea   rdx, [rel texto1]
    call  qword [rdi + 8]
...
texto1:                           ; UTF-16: "kernel-1.5.2026" + quebra de linha
    dw 'k','e','r','n','e','l','-','1','.','5','.','2','0','2','6',13,10
    dw 0

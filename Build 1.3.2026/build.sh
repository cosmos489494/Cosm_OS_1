...
#             kernel-1.3.2026       (na raiz do projeto - SEPARADO)
...
SRC="inicio.asm"
KRN_SRC="kernel.asm"
KRN="kernel-1.3.2026"          # o nome carrega a versao da build
VID_SRC="video.asm"
...
echo "== [1/5] NASM: $SRC -> inicio.obj (COFF x64)"
nasm -f win64 "$SRC" -o inicio.obj

echo "== [1/5] LINKER: inicio.obj -> $OUT_EFI (.efi UEFI)"
mkdir -p EFI/Boot
lld-link /machine:x64 /subsystem:efi_application /entry:inicio \
         /out:"$OUT_EFI" inicio.obj

echo "== [2/5] NASM: $KRN_SRC -> $KRN (binario flat - arquivo SEPARADO)"
nasm -f bin "$KRN_SRC" -o "$KRN"

echo "== [3/5] NASM: $VID_SRC -> $VID (driver de video - binario flat)"
nasm -f bin "$VID_SRC" -o "$VID"

echo "== [4/5] Montando imagem FAT da ESP (só o bootx64.efi, El Torito UEFI)"
...
echo "== [5/5] XORRISO: gerando $ISO (EFI/Boot + $KRN + $VID lado a lado)"

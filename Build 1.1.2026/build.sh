...
#
# Cosm OS - build.sh
# Compila o fonte Assembly e monta a ISO UEFI bootavel.
#
#   entrada : inicio.asm
...
echo "== [1/3] NASM: $SRC -> inicio.obj (COFF x64)"
nasm -f win64 "$SRC" -o inicio.obj

echo "== [1/3] LINKER: inicio.obj -> $OUT_EFI (.efi UEFI)"
mkdir -p EFI/Boot
lld-link /machine:x64 /subsystem:efi_application /entry:inicio \
         /out:"$OUT_EFI" inicio.obj

echo "== [2/3] Montando imagem FAT da ESP (pro El Torito UEFI)"
...
echo "== [3/3] XORRISO: gerando $ISO (unico arquivo: $OUT_EFI)"
